...
if [[ -d "$INSTALL_DIR" ]]; then
  ...
  # Preserve venv to avoid re-downloading packages
  if [[ -d "$INSTALL_DIR/venv" ]]; then
    mv "$INSTALL_DIR/venv" /tmp/ensemble_venv_backup
  fi
...
if [[ -d /tmp/ensemble_venv_backup ]]; then
  mv /tmp/ensemble_venv_backup "$INSTALL_DIR/venv"
  info "Restored existing venv"
fi
...
./venv/bin/pip install --quiet rumps keyring
