...
class GitService:
    ...
    def ensure_worktree_base(self):
        ...
        if os.path.exists(gitignore):
            ...
            if ".wt/" not in content:
                additions.append(".wt/")
            if "docs/plan.md" not in content:
                additions.append("docs/plan.md")
            if additions:
    ...
    def commit_worktree(self, name: str, message: str) -> tuple[bool, str]:
        ...
        # Remove plan docs — they're working artifacts, not code changes
        for plan_file in ["docs/plan.md", "docs/PLAN.md", "plan.md"]:
            plan_path = os.path.join(wt_path, plan_file)
            if os.path.exists(plan_path):
                os.remove(plan_path)
                # Remove empty docs/ dir too if nothing left
                docs_dir = os.path.dirname(plan_path)
