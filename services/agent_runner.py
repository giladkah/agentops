...
class AgentRunner:
    ...
    def launch_agent(
        ...
        if self._mode == "api":
            return self._launch_api(agent_id, worktree_path, prompt, model, on_complete, app, buf)
        else:
            return self._launch_cli(agent_id, worktree_path, prompt, model, on_complete, app, buf)

    # ── API Backend (streaming + tool use) ──

    def _launch_api(self, agent_id, worktree_path, prompt, model, on_complete, app, buf):
        ...
        def _run():
            ...
            try:
                ...
                while turns < MAX_AGENTIC_TURNS:
                    ...
                    buf.add("error", {"message": f"Hit max turns ({MAX_AGENTIC_TURNS})"})
                    all_text.append(f"\n⚠️ Max turns reached")
    ...
    def _launch_cli(self, agent_id, worktree_path, prompt, model, on_complete, app, buf):
